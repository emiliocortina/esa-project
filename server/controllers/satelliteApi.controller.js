  ...
  ramani.getPoint(
    ...
    function(err, ret) {
      ...
      let datapack = dataObject.map(value => {
        let dateInCeroOne =
          (Date.parse(value.time) - firstDate) / (lastDate - firstDate);
...
exports.getStartAndEndDate = async (req, res, next) => {
  ...
  res
    ...
    .json({ start: minimunStartTime, end: maximunEndTime });
  return;
};
...
exports.handleLayerCall = async (req, res, next) => {
  ...
  try {
    ...
    if (satRequestDto.end == null) {
      ...
      if (
        parsedDate.getTime() < minimunStartTime.getTime() ||
        parsedDate.getTime() > maximunEndTime.getTime()
      ) {
        next(
          errorServ.buildError(
            req.url,
            HttpStatus.NOT_FOUND,
            "no_data",
            "No data available on that time"
          )
        );
        return;
      } else {
      ...
      if (
        parsedStart.getTime() > maximunEndTime.getTime() ||
        parsedEnd.getTime() < minimunStartTime.getTime()
      ) {
        next(
          errorServ.buildError(
            req.url,
      ...
      time[
        "formalized"
      ] = `${parsedStart.toISOString()}/${parsedEnd.toISOString()}`;
