...
const mapService = require("../services/opticMaps.service");
...
exports.createThread = async (req, res, next) => {
  ...
  if (location) {
    let imageUrl = await mapService.generateMap(
      location.latitude,
      location.longitude
    );
...
exports.findThreadsPaginated = async (req, res, next) => {
  ...
  if (sortAndFilterInfo.sortFields[0]) {
    ...
    var order =
      sortAndFilterInfo.sortFields[0].order == PaginationModule.ASC ? 1 : -1;
    sort[key] = order;
  }
  ...
  if (userEmail) {
    ...
    if (user) {
      ...
      if (sortAndFilterInfo.sortFields[0]) {
        var key = sortAndFilterInfo.sortFields[0].key;
        var order =
          sortAndFilterInfo.sortFields[0].order == PaginationModule.ASC
            ? 1
            : -1;
        sort[key] = order;
      }
      ...
      const threads = await Thread.find({ author: userId })
        .sort(sort)
        .skip(
          (sortAndFilterInfo.page_number - 1) * sortAndFilterInfo.page_elements
        )
        .limit(sortAndFilterInfo.page_elements)
        .exec()
        .then(
